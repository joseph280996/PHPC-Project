...
ROTATIONTYPE parseRotationType(const string &type) {
    if (type == "c" || type == "clockwise") {
        ...
        throw invalid_argument(
            "Invalid rotation type. Use 'c'/'clockwise' or 'cc'/'counterclockwise'");
    }
}
...
void rotate_parallel_clockwise(const uchar *input_data, uchar *output_data, int in_rows, int in_cols, int channels, int rank, int num_processes) {
...
void rotate_parallel_counterclockwise(const uchar *input_data, uchar *output_data, int in_rows, int in_cols, int channels, int rank, int num_processes) {
...
int main(int argc, char **argv) {
    if (argc != 3) {
        cout << "Usage: " << argv[0] << " <image_path> <rotation_type>" << endl;
        cout << "rotation_type: 'c' or 'clockwise' for clockwise rotation" << endl;
        cout << "               'cc' or 'counterclockwise' for counterclockwise rotation" << endl;
        return -1;
    }
    ...
    if (rank == 0) {
        ...
        cout << "Sequential time: "
             << duration_cast<microseconds>(stop_seq - start_seq).count() << " microseconds"
             << endl;

        string rot_str = (rotationtype == CLOCKWISE) ? "clockwise" : "counterclockwise";
        string seq_out_path = "output/sequential/sequential_" + rot_str + "_result.jpg";
        imwrite(seq_out_path, seqOutput);
    ...
    size_t out_total_size = (rank == 0) ? out_dims[0] * out_dims[1] * out_dims[2] : 0;

    MPI_Win_allocate_shared(in_total_size, 1, MPI_INFO_NULL, nodeComm, &sharedInData, &in_win);
    MPI_Win_allocate_shared(out_total_size, 1, MPI_INFO_NULL, nodeComm, &sharedOutData, &out_win);

    if (rank != 0) {
        MPI_Aint in_shared_size, out_shared_size;
        int in_disp_unit, out_disp_unit;
        MPI_Win_shared_query(in_win, 0, &in_shared_size, &in_disp_unit, &sharedInData);
        MPI_Win_shared_query(out_win, 0, &out_shared_size, &out_disp_unit, &sharedOutData);
    ...
    if (rotationtype == CLOCKWISE) {
        rotate_parallel_clockwise(sharedInData, sharedOutData, in_rows, in_cols, in_ch, rank, num_processes);
    } else {
        rotate_parallel_counterclockwise(sharedInData, sharedOutData, in_rows, in_cols, in_ch, rank, num_processes);
    }
    ...
    if (rank == 0) {
        ...
        cout << "Parallel time: "
             << duration_cast<microseconds>(stop_par - start_par).count() << " microseconds"
             << endl;
        ...
        string rot_str = (rotationtype == CLOCKWISE) ? "clockwise" : "counterclockwise";
        string par_out_path = "output/parallel/parallel_" + rot_str + "_result.jpg";
        imwrite(par_out_path, result);
    }
